...
3. 流程与架构图绘制 - `4flow-diagram`
4. 图表实验室（多工具画图） - `7figure-lab`
5. 竞赛论文撰写 - `5writing`
6. 论文打磨与升华 - `8polish`
7. 验证和验收 - `6verity`
```
...
│   ├── DIAGRAM_REPORT.md             # 3: 非数据图说明（4flow-diagram）
│   ├── FIGURES_MENU.md              # 4: 图单（7figure-lab，供最终挑选）
│   ├── POLISH_REPORT.md             # 6: 论文打磨报告（8polish）
│   ├── VERIFY_REPORT.md             # 7: 验收报告（6verity）
...
├── figures/                     # 2+3+4: 所有图表（3coding-visual + 4flow-diagram + 7figure-lab）
│   ├── *.pdf                    #     数据图 + 非数据图 + 示意图 PDF
│   ├── *.html                  #     非数据图源文件（4flow-diagram，HTML 内嵌 SVG）
│   ├── *.html                   #     HTML 交互图 / SVG 源（7figure-lab）
├── paper/                       # 5: 论文（5writing）
...
| 流程与架构图绘制 | `4flow-diagram` | 在论文确实需要时，绘制方法流程图、架构图和非数据型概念图。 | `figures/*.html`, `figures/*.pdf`, `DIAGRAM_REPORT.md` |
| 图表实验室（多工具） | `7figure-lab` | 绘制几何/坐标/参数关系示意图和 HTML 交互图，对关键图用 Python/HTML 各出一版，输出图单供挑选。 | `figures/*.pdf`, `figures/*.html`, `FIGURES_MENU.md` |
| 竞赛论文撰写 | `5writing` | 基于分析、建模、代码结果和图表撰写最终竞赛论文，并按章节直接插入图表。 | `paper/` |
| 论文打磨与升华 | `8polish` | 国奖范文风格校准、亮点挖掘、自我评审与局部回炉重写，让论文从结构完整跃升到国奖水准。 | `POLISH_REPORT.md` |
| 验证和验收 | `6verity` | 检查可复现性、一致性、产物完整性、格式规范和提交就绪状态。 | `VERIFY_REPORT.md` |

## 阶段边界

- `3coding-visual` 负责生成所有依赖计算结果或实验输出的数据图表。
- `4flow-diagram` 只负责概念图、算法流程图、架构图、路线图等非数据型图示。
- 不要让 `4flow-diagram` 重复绘制 `3coding-visual` 已经生成的统计图或数据图。
- `7figure-lab` 负责几何/坐标/参数关系示意图、HTML 交互图，以及关键图的多工具多版本生成与图单汇总；不重跑模型、不改数值结论。
- `5writing` 负责决定图表在论文中的位置，并按 LaTeX 语法写入图表代码：
  - `\begin{figure}[H]\centering\includegraphics[width=0.85\textwidth]{../../figures/xxx.pdf}\caption{...}\label{fig:xxx}\end{figure}`
- 不要让 `5writing` 编造数值结论。论文中的数值必须来自 `RESULTS_REPORT.md`、结果表或已生成图表的数据。
- `8polish` 在 `5writing` 之后、`6verity` 之前运行：只做局部打磨（风格校准、亮点提炼、回炉重写），不重跑模型、不改数值结论；数值仍以 `RESULTS_REPORT.md` 为准。
