...
Options:
  ...
  --main FILE            Paper entry file (LaTeX main.tex). Defaults to <paper-dir>/main.tex.
  --sections-dir DIR     Section directory. Defaults to <paper-dir>/sections when it exists.
  --references FILE      Reference file. Defaults to <paper-dir>/references.tex when it exists.
...
if [ -z "$PAPER_DIR" ]; then
  if [ -f "main.tex" ]; then
    ...
    CANDIDATE="$(find . -maxdepth 3 -type f -name main.tex -print 2>/dev/null | head -n 1 || true)"
    if [ -n "$CANDIDATE" ]; then
...
if [ -z "$REFERENCES_FILE" ]; then
  if [ -f "$PAPER_DIR/references.tex" ]; then
...
if not section_files:
    warn(f"no separate section {section_ext} files detected; treating paper as a single-file LaTeX document")

# LaTeX \input{...} or \include{...} detection
...
for name in include_names:
    if name in seen:
        fail(f"duplicate LaTeX include: {name}")
    seen.add(name)

for inc, path in zip(includes, include_paths):
    if not path.exists():
        fail(f"included LaTeX file does not exist: {inc}")
...
ai_buzzwords = re.compile(
    r"首先|其次|再次|最后|综上所述|值得注意的是|不难发现|不难看出|众所周知|"
    r"具有重要意义|深远意义|充分发挥|深入剖析|深刻理解|全面系统|有效提升|显著改善|"
    r"随着科技的进步|在当今.*背景下"
)
...
tex_files = unique_paths([main] + section_files + ([refs] if refs and refs.exists() else []) + sorted(paper.glob(section_ext)))
file_texts = []
combined = []
section_titles = []
...
for path in tex_files:
    ...
    if path in section_files:
        ...
        ai_buzz = len(ai_buzzwords.findall(text))
        if ai_buzz >= 8:
            warn(f"many AI-flavored phrases, consider plain prose: {path.name} ({ai_buzz})")
...
# Image reference check: LaTeX \includegraphics{...}
