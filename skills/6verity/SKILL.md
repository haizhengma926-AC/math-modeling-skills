---
name: 6verity
description: "数学建模竞赛最终验证和验收阶段（LaTeX）。用于论文写完后检查章节数量、标题顺序、图表引用、数值一致性、占位符、内部文件泄露、参考文献、代码可复现性、编译和提交就绪状态。"
...
优先运行本 skill 的脚本（针对 LaTeX 的 `.tex` 入口做检查）：
...
- 过多列表式写作（LaTeX 中大量 `\begin{itemize}`、`\begin{enumerate}`）。
...
如果是模板转换或已有参考 PDF 的项目，还应将新旧 PDF 都逐页导出 PNG，按页对比版式差异；页数或页面尺寸不一致必须记录为硬错误或明确说明原因。
...
- 缺少选定的论文入口文件（`main.tex`）或核心正文。
- 论文入口引用的章节文件不存在。
- LaTeX 入口缺少 `\input`/`\include`。
- 正文章节缺少一级标题（LaTeX `\section{}` 缺失）。
- 章节顺序明显错误或重复。
- 正文仍有占位符。
- 正文泄露内部工作流文件名。
- 引用的图片不存在。
