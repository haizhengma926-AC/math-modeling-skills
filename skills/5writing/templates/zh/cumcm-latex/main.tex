...
%    - 本模板默认使用 ctex 的 fontset=windows，在 Windows 上直接命中
%      SimSun / SimHei / KaiTi，确保开箱即用。
%    - 若在 macOS 编译，请将 \documentclass 的 fontset=windows 改为 fontset=mac，
%      以使用 Songti SC / Heiti SC / Kaiti SC。
%    - 西文正文：Times New Roman；代码等宽：Menlo（缺失时回退到默认字体）。
%  编译方式：xelatex main.tex （需运行两遍以解决交叉引用与页码）
% =============================================================================
...
% 中文宋体/黑体/楷体由 ctex 的 fontset=windows 自动设置（SimSun / SimHei / KaiTi），
