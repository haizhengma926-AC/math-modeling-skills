...
| [图表与可视化](#图表与可视化) | `3coding-visual` `4flow-diagram` `5writing` |
| [非数据图工具选择](#非数据图工具选择) | `4flow-diagram` `5writing` |
| [论文写作](#论文写作) | `5writing` |
...
1. 图由真实数据/计算结果生成 → `3coding-visual` 用 Python（matplotlib / seaborn / plotly）。
2. 图含数学公式/符号需精确标注 → 用 TikZ 在 LaTeX 正文中直接绘制（与正文同字体、公式可引用）。
3. 纯结构/流程/概念、无公式 → `4flow-diagram` 用 HTML（内嵌 SVG），导出矢量 PDF。

- HTML（内嵌 SVG）适合技术路线图、子问题求解流程图、数据处理 Pipeline、指标体系层次图、模型选择决策树、甘特图、小规模网络拓扑和简单概念框架，可导出矢量 PDF 供论文引用。
...
- 速查：需要公式或精确连线时优先 TikZ；需要可编辑流程结构时优先 HTML（内嵌 SVG）；需要真实场景或复杂 3D 视觉时才考虑生成式图片。
...
- 核心公式引出前必须先用文字交代问题背景、变量定义和推导依据，禁止用一两句话直接引出公式；公式之间用逻辑关系衔接，不要写成“一句话 → 公式 → 一句话 → 公式”的拼接，每个重要公式要回答“描述什么、为什么需要、变量含义”。
