...
| `msedge` / `chrome`（无头浏览器） | HTML 流程图导出 PDF（4flow-diagram） | `command -v msedge` 等，或查 Program Files 下的 Edge/Chrome |
...
command -v msedge >/dev/null 2>&1 || command -v chrome >/dev/null 2>&1 || command -v chromium >/dev/null 2>&1 \
  || [ -f "/c/Program Files (x86)/Microsoft/Edge/Application/msedge.exe" ] \
  || [ -f "/c/Program Files/Google/Chrome/Application/chrome.exe" ] \
  && echo "OK  html2pdf(无头浏览器)" || echo "MISS html2pdf(无头浏览器)"
check_cmd pdftoppm
check_cmd mutool
check_cmd magick
...
状态   工具/包              说明
----   --------             ----
✗      html2pdf              HTML 转 PDF（可选，无头浏览器/cairosvg）
✓      python3 3.11.x       数值计算
✗      scipy                科学计算（可选）
...
```

**必须项：** xelatex、python3、numpy、pandas、matplotlib  
**可选项：** 无头浏览器（HTML 转 PDF）、pdftoppm/mutool/magick 三选一、scipy、scikit-learn、openpyxl
...
#### HTML 转 PDF 工具（无头浏览器 / cairosvg）

流程图用 HTML（内嵌 SVG）绘制，需要转成矢量 PDF 供论文引用。优先用系统自带的无头浏览器，不可用时用 Python 的 cairosvg。

| 平台 | 命令 |
| --- | --- |
| Windows | 自带 Edge，无需安装；或 `winget install Google.Chrome` |
| macOS | `brew install --cask google-chrome` |
| Linux | `sudo apt install chromium-browser`（Debian/Ubuntu）或 `sudo dnf install chromium` |
...
Doctor 检查完成（macOS）
必须项：5/5 ✓
可选项：2/4（无头浏览器、scipy 缺失）

工作流就绪状态：
  1start-mathmodel   ✓
  2analysis-modeling ✓
  3coding-visual     ✓（scipy 缺失，部分功能受限）
  4flow-diagram            ⚠ 无头浏览器/cairosvg 未安装，PDF 导出将跳过
  7figure-lab        ✓（Python 绘图 + HTML 交互图）
  5writing           ✓（xelatex ✓）
...
- HTML 转 PDF 工具和 PDF 转 PNG 工具缺失不影响核心工作流，仅影响导出质量。
